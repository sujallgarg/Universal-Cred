        ...
        if (cachedBalanceStr !== null) {
            ...
            if (dbRes.rows.length > 0 && dbRes.rows[0]) {
                ...
                try {
                    ...
                    const insertRes = await dbClient.query(`INSERT INTO wallets (user_id, balance) 
             VALUES ($1, $2) 
             ON CONFLICT (user_id) DO UPDATE SET balance = wallets.balance 
             RETURNING id, balance`, [userId, defaultBalance]);
                    await dbClient.query("COMMIT");
                    const row = insertRes.rows[0];
                    if (!row)
                        throw new Error("Failed to retrieve wallet id after insert.");
                    walletId = row.id;
