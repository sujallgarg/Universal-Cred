...
router.get("/balance", async (req, res) => {
    ...
    try {
        ...
        if (balanceStr === null || walletIdStr === null) {
            // Lazy load from DB
            const dbRes = await pool.query("SELECT w.id, w.balance FROM wallets w JOIN users u ON w.user_id = u.id WHERE u.id = $1", [userId]);
            if (dbRes.rows.length > 0 && dbRes.rows[0]) {
                ...
                try {
                    ...
                    await dbClient.query("INSERT INTO users (id, email, password_hash) VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING", [userId, `user_${userId}@example.com`, "bcrypt_hash"]);
                    const defaultBalance = 500000000n; // $500.00 USD
                    const insertRes = await dbClient.query("INSERT INTO wallets (user_id, balance) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET balance = wallets.balance RETURNING id, balance", [userId, defaultBalance]);
                    await dbClient.query("COMMIT");
                    const row = insertRes.rows[0];
                    if (!row)
                        throw new Error("Database wallet insertion failed.");
    ...
    try {
        ...
        try {
            ...
            if (dbRes.rows.length === 0) {
                await dbClient.query("INSERT INTO users (id, email, password_hash) VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING", [userId, `user_${userId}@example.com`, "bcrypt_hash"]);
                const insertRes = await dbClient.query("INSERT INTO wallets (user_id, balance) VALUES ($1, $2) RETURNING id, balance", [userId, topupAmount]);
                const row = insertRes.rows[0];
                if (!row)
                    throw new Error("Wallet creation failure.");
                walletId = row.id;
            }
            else {
                const wallet = dbRes.rows[0];
                if (!wallet)
                    throw new Error("Wallet row empty.");
                walletId = wallet.id;
                oldBalance = BigInt(wallet.balance);
                await dbClient.query("UPDATE wallets SET balance = balance + $1, updated_at = NOW() WHERE id = $2", [topupAmount, walletId]);
            }
...
router.get("/ledger", async (req, res) => {
    ...
    try {
        const ledgerRes = await pool.query(`SELECT le.* 
