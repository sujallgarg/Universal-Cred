  ...
  `;
    try {
        ...
        if (!useDbMemoryFallback) {
            ...
            await pool.query("INSERT INTO users (id, email, password_hash) VALUES (98765, 'developer@example.com', 'dev_hash') ON CONFLICT (id) DO NOTHING");
            await pool.query("INSERT INTO wallets (user_id, balance) VALUES (98765, 500000000) ON CONFLICT (user_id) DO UPDATE SET balance = CASE WHEN wallets.balance < 500000000 THEN 500000000 ELSE wallets.balance END");
